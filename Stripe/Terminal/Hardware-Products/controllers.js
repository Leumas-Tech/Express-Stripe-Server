...
exports.getHardwareProductById = async (req, res) => {
    const { id } = req.params;

    try {
        const response = await axios.get(`${BASE_URL}/hardware_products/${id}`, {
            headers: HEADERS,
        });
...
exports.getAllHardwareProducts = async (req, res) => {
    try {
        const response = await axios.get(`${BASE_URL}/hardware_products`, {
            headers: HEADERS,
        });
